...
try {
    ...
    $tab_name             = isset($input['tabName']) ? $input['tabName'] : 'refunded';
    $customer_fil         = "";
    $booking_filter       = "";

    // Build travel agency query based on user type with GROUP BY
    if ($userType == '24') {
    ...
    $sql = "SELECT b.id,
            ...
            WHERE b.ta_id IN ($ta_ids_str) AND b.status='3' 
            AND b.date $booking_filter
            $customer_fil 
    ...
    foreach ($bookings as $booking) {
        ...
        // Calculate payment % using data from main query
        $perecent_fill = 0;
        $booking_paid_amt = 0;
    ...
    echo json_encode(["status" => "success", "bookings" => $response]);
