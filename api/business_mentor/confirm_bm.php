...
$operation = "Confirm";
	...
	if ($result) {
		...
		if($result2){
			...
			if($result3){
				...
				if(!$mail->Send()){
					...
					echo json_encode(["status" => "success", "message" => "Business Mentor registered. Login detailed mailed to the recipient. Kindly check spam in case not received."]);
				}
			}else{  //email
		...
		echo json_encode(["status" => "success", "message" => "Business Mentor registration failed"]);
	}
