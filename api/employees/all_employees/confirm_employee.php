...
if ($row9) {
    ...
    echo json_encode(["status" => 1, "message" => "Employee found"]);
} else {
    echo json_encode(["status" => 0, "message" => "Employee not found or invalid status"]);
    exit;
}

$sql2 = $conn->prepare("SELECT distinct employee_id, SUBSTRING(employee_id, 3, 6) as tc_id FROM employees WHERE status IN ('1', '3') ORDER BY tc_id DESC LIMIT 1");
$sql2->execute();
        ...
        if($result3){
			...
			$mail->SMTPOptions=array('ssl'=>array(
				...
				if(!$mail->Send()){
					...
					echo json_encode(["status" => "success", "message" => "Employee registered. Login detailed mailed to the recipient. Kindly check spam in case not received."]);
				}
			}else{  //email
				echo json_encode(["status" => "error", "message" => "logs could'nt be inserted"]);
			}
		}else{ //logs
			echo json_encode(["status" => "error", "message" => "login details could'nt be updated"]);
		}

	}else{ //login
		echo json_encode(["status" => "success", "message" => "Employee registration failed"]);
	}
?>
