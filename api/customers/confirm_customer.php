...
$sql1 = "UPDATE ca_customer SET status=:status,ca_customer_id=:ca_customer_id, register_date=:register_date WHERE id=:id";
	...
	if ($result) {
		...
		if($result2){
			...
			if($result3){
				...
				if(!$mail->Send()){
					...
					echo json_encode(["status" => "success", "message" => "Customer registered. Login detailed mailed to the recipient. Kindly check spam in case not received."]);
				}
			}
			else{
		...
		echo json_encode(["status" => "success", "message" => "Customer registration failed"]);
	}
