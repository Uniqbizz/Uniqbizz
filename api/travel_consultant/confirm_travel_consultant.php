...
if ($result) {
			...
			if($result4){
				...
				if($result5){
					...
					if(!$mail->Send()){
						...
						echo json_encode(["status" => "success", "message" => "Travel Consultant registered. Login detailed mailed to the recipient. Kindly check spam in case not received."]);
					}
				}else{
					echo 0	;
				}
	...
	echo json_encode(["status" => "success", "message" => "Travel Consultant registration failed"]);
}
?>