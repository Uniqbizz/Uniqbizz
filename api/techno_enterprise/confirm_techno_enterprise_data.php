        ...
        if ($result3) {
            ...
            if ($result4) {
				...
				if(!$mail->Send()){
					...
					echo json_encode(["status" => "success", "message" => "Techno Enterprise registered. Login detailed mailed to the recipient. Kindly check spam in case not received."]);
				}
                } else {
                    echo json_encode(["status" => "error", "message" => "Failed to send email"]);
                }
            } else {
                echo json_encode(["status" => "error", "message" => "Failed to process commission"]);
            }
        } else {
            echo json_encode(["status" => "error", "message" => "Failed to insert logs"]);
        }
    } else {
        echo json_encode(["status" => "error", "message" => "Failed to insert login details"]);
    }
} else {
    echo json_encode(["status" => "error", "message" => "Failed to update Techno Enterprise"]);
}