...
<?php
    ...
    if($totalAmountMessage){
        $stmt = " SELECT SUM(cte_amount) as TotalPayout FROM techno_enterprise_payout WHERE cte_id = '".$userID."' AND YEAR(created_date) = '".$TotalYear."' AND MONTH(created_date) = '".$TotalMonth."' ";
    ...
    if($totalTableMessage){
        echo '<table class="table table-hover table-responsive" id="totalPayoutTable">
            ...
            <tbody >';
                ...
                $model2 = "SELECT * FROM techno_enterprise_payout WHERE cte_id = '".$userID."' AND YEAR(created_date) = '".$TotalYear."' AND MONTH(created_date) = '".$TotalMonth."'";
                ...
                if($model2 -> rowCount()>0){
                    foreach($model2 -> fetchAll() as $key => $row){
                        ...
                        $message = $row['cte_message'];
                        $message =  str_replace('.','<br>',$message);  

                        // total Amt Cal for BC 
                        $CommAmt = $row['cte_amount'];
                        $tds = $CommAmt * 2/100;
                        ...
                        echo'<tr>
                            ...
                            <a href="payout/forms/contracting_payout/download_ca_payout.php?vkvbvjfgfikix='.$row['id'].'&bc='.$row['cte_id'].'&ca='.$row['te_id'].'&date='.$dt.'&message='.$message.'&message_status='.$row['status'].'&commission='.$row['cte_amount'].'">
