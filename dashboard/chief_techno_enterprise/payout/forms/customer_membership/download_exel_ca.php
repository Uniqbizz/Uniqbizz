    ...
    if($stmt2->rowCount() > 0){
        ...
        foreach($stmt2->fetchAll() as $row2){
            ...
            if($user_id_str == "ST"){
                ...
                $amount = $row2['commision_cte'];
                $tds = round($amount * 2 / 100, 2);
                $total = $amount - $tds;

                $message = str_replace('.', PHP_EOL, $row2['message_cte']);
                ...
                $statusText = ($row2['status_cte'] == 1)
                    ? 'Paid'
                    : 'Pending';

                $paidDate = ($row2['status_cte'] == 1)
                    ? $row2['paydate']
                    : 'NA';
    ...
    if($stmt2->rowCount() > 0){
        ...
        foreach($stmt2->fetchAll() as $row2){
            ...
            if($user_id_str == "ST"){
                ...
                $message = str_replace('.', ' ', $row2['message_cte']);
                ...
                $status = ($row2['status_cte'] == 1) ? 'Paid' : 'Pending';
                $paidDate = ($row2['status_cte'] == 1)
                    ? $row2['paydate']
                    : 'NA';
...
if($payoutmessage == 'allPayout'){
    ...
    //     $stmt2 = " SELECT * FROM ca_cu_payout WHERE cte_id = '".$user_id."' AND YEAR(created_date) = '".$payoutYear."' AND MONTH(created_date) = '".$payoutMonth."' ";
    ...
    if($stmt2 -> rowCount()>0){
        ...
        <table border="1" style="text-align:center">
            ...
            foreach($stmt2->fetchAll() as $key => $row2){
                ...
                if($user_id_str == "SF"){
                    $sql1= $conn->prepare("SELECT firstname,lastname FROM `sponsor_franchisee` where sponsor_franchisee_id='".$row2['cte_id']."'");
                }else if($user_id_str == "MF"){
                    $sql1= $conn->prepare("SELECT firstname,lastname FROM `master_franchisee` where master_franchisee_id='".$row2['cte_id']."'");
                }else{
                    $sql1= $conn->prepare("SELECT firstname,lastname FROM `cte_id` where cte_id_id='".$row2['cte_id']."'");
                }
                $sql1->execute();
