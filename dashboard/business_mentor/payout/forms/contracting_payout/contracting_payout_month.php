...
<?php
    ...
    if($totalAmountMessage){
        $stmt = " SELECT SUM(comm_amt) as TotalPayout FROM goa_bm_payout WHERE bm_id= '".$userID."' AND YEAR(created_date) = '".$TotalYear."' AND MONTH(created_date) = '".$TotalMonth."' ";
    ...
    if($totalTableMessage){
        echo '<table class="table table-hover table-responsive" id="totalPayoutTable">
            ...
            <tbody >';
                ...
                $model2 = "SELECT * FROM goa_bm_payout WHERE bm_id= '".$userID."' AND YEAR(created_date) = '".$TotalYear."' AND MONTH(created_date) = '".$TotalMonth."'";
                ...
                if($model2 -> rowCount()>0){
                    foreach($model2 -> fetchAll() as $key => $row){
                        ...
                        echo'<tr>
                            ...
                            <a href="payout/forms/contracting_payout/download_ca_payout.php?vkvbvjfgfikix='.$row['id'].'&bc='.$row['bm_id'].'&ca='.$row['techno_enterprise'].'&date='.$dt.'&message='.$message.'&message_status='.$row['status'].'&commission='.$row['comm_amt'].'">
