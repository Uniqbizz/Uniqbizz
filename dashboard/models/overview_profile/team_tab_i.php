...
<div class="tab-pane fade pb-4 rounded-4" id="teams" role="tabpanel"> 
    <?php
        ...
        $stmt2_3 = $conn -> prepare(" SELECT * FROM institution_branch_manager WHERE reference_no = ? AND status = '1' ORDER BY institution_branch_manager_id ASC");
        $stmt2_3 -> execute([$id]);
        $referrals2_3 = $stmt2_3->fetchAll(PDO::FETCH_ASSOC);
        foreach($referrals2_3 as $referral2){
            $cas_id = $referral2['institution_branch_manager_id'];
    ?>
                ...
                <div class="row align-items-center">
                    ...
                    <div class="col-lg-2 col-md-2 col-sm-6 col-6 py-2">
                        <div class="text-center">
                            <a href="#" onclick="overviewPage('<?= $referral2['institution_branch_manager_id'] .','.  $referral2['reference_no'] . ',' .$referral2['country']. ',' .$referral2['state']. ',' .$referral2['city']. ',travel_consultant' ?>')" class="btn btn-primary view-btn">View Profile</a>
                        </div>
                    </div>
                </div>
            </div>
