    ...
    if( $stmt -> rowCount()>0 ){
        foreach( ($stmt -> fetchALL()) as $key => $row ){
            ...
            echo '<tr>
                    ...
                    <td >'.$tds.'</td>
                    <td >'.$total.'
                    <a href="../controllerspayout/forms/sub_franchisee_payout/download_ca_payout.php?vkvbvjfgfikix='.$row['id'].'&bc='.$id.'&ca='.$row['sub_franchisee'].'&designation='.$columnDesignation.'&date='.$dt.'&message='.$message.'&message_status='.$status.'&commission='.$amt.'">
