    ...
    if($stmt2 -> rowCount()>0){
        ...
        <table border="1" style="text-align:center">
            ...
            foreach($stmt2->fetchAll() as $key => $row2){
                ...
                if($user_id_str == "SF" || $user_id_str == "MF"){
...
if($payoutmessage == 'allPayout'){
    if($user_id_str == 'SF' || $user_id_str == 'MF'){
        $stmt2 = " SELECT * FROM sub_franchisee_payout WHERE business_mentor = '".$user_id."' AND YEAR(created_date) = '".$payoutYear."' AND MONTH(created_date) = '".$payoutMonth."' ";
    }
        ...
        <table border="1" style="text-align:center">
            ...
            foreach($stmt2->fetchAll() as $key => $row2){
                ...
                if($user_id_str == "SF"){
                    ...
                    $sql1= $conn->prepare("SELECT firstname,lastname FROM `business_mentor` where business_mentor_id='".$row2['master_franchisee']."'");
                }
                $sql1->execute();
                $sql1->setFetchMode(PDO::FETCH_ASSOC);
                if($sql1->rowCount()>0){
