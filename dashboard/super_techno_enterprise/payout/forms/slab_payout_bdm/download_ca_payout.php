...
if ($userType == 25) {
    ...
    if (!empty($ids) && $ids[0] !== "") {
        ...
        while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
            $names[] = $row['name'] . " (" . $row['business_mentor_id'] . ")";
        }
    }

    // Append Active BMs
    if (!empty($names)) {
        $message .= " Active BMs: " . implode(', ', $names) . ".";
    }

    // Debugging Output
    echo '<script>console.log("' . addslashes($message) . '")</script>';
    //echo "Final Message: " . $message; // Ensure it's actually appended
} else if ($userType == 24) {
    $bcNames = $conn->prepare("SELECT * FROM employees WHERE employee_id = '" . $bc . "' AND user_type=24 AND status = 1");
    $bcNames->execute();
    $bcNames->setFetchMode(PDO::FETCH_ASSOC);
    if ($bcNames->rowCount() > 0) {
        foreach (($bcNames->fetchAll()) as $key => $row) {
    ...
    if (!empty($ids) && $ids[0] !== "") {
        $placeholders = rtrim(str_repeat('?,', count($ids)), ','); // ?,?,?
        $query = $conn->prepare("SELECT employee_id, name FROM employees WHERE employee_id IN ($placeholders) AND status = 1 AND user_type=25 ");
        $query->execute($ids);

        while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
            $names[] = $row['name'] . " (" . $row['employee_id'] . ")";
        }
    }

    // Append Active BDMs
    if (!empty($names)) {
        $message .= " Active BDMs: " . implode(', ', $names) . ".";
    }
}
        ...
        <div class="container cont-btn d-flex justify-content-around pt-3 pb-4">
            <a href="../../../<?php
                                if ($userType == 25) {
                                    ...
                                    echo 'bcm_recruitement_payout_slab';
                                } ?>.php" class="go-back"> Go Back</a>
                ...
                <div class="col-md-12 col-sm-12 col-12 ps-3 pe-3" style=" ">
                    <div class="row">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td class="col-md-5 col-sm-5 pt-3">
                                        <h6 style="padding:2px 0; font-weight: 700;">Pay For : <?php
                                                                                                if ($userType == 25) {
                                                                                                    echo 'Business Mentor';
                                                                                                } else if ($userType == 24) {
                                                                                                    echo 'Business Development Manager';
                                                                                                } ?></h6>
                                        <h6 style="padding:2px 0; font-weight: 700;">Designation : <?php
                                                                                                    if ($userType == 25) {
                                                                                                        echo 'Business Development Manager';
                                                                                                    } else if ($userType == 24) {
                                                                                                        echo 'Business Channel Manager';
                                                                                                    } ?></h6>
                        ...
                        <div class="col-md-12 col-sm-12" style="">
                            <h5 style="padding: 10px 5px;  margin:0px; font-weight: 700; "><?php
                                                                                            if ($userType == 25) {
                                                                                                echo 'Business Mentor';
                                                                                            } else if ($userType == 24) {
                                                                                                echo 'Business Development Manager';
