...
//get username based on designation changed for All payout --no use--
...
//display username for filter perpose for All payout --no use--
$('#user_id_name').on('change', function(){
    ...
    $.ajax({
        type: 'POST',
        url: 'forms/customer_membership/recruitment_payout_filter.php',
...
//date split for filter for All payout --no use--
...
//get username based on designation changed for Previous Payout --no use--
...
//display username for filter perpose for Previous Payout --no use--
...
// Previous payout filter option select display name and amout below  --no use--
$('#user_id_namePrevious').on('change', function(){
    cap_id = $(this).val();
    year_split = $("#prevYear").val();
    month_split = $("#prevMonth").val();
    ...
    $.ajax({
        type: 'POST',
        url: 'forms/customer_membership/recruitment_payout_filter_amt_user.php',
        data: dataString,
        cache: false,
        success: function(data){
            $('#prevDiv').html(data);
...
//display username for filter perpose for Next Payout --no use--
...
// Next payout filter option select display name and amout below  --no use--
$('#user_id_nameNext').on('change', function(){
    cap_id = $(this).val();
    year_split = $("#nextYear").val();
    month_split = $("#nextMonth").val();
...
//get username based on designation changed for Total Payout --no use--
...
//display username for filter perpose for Total Payout --no use--
...
// Total payout filter option select display name and amout below  --no use--
$('#user_id_nameTotal').on('change', function(){
    cap_id = $(this).val();
    year_split = $("#nextYear").val();
    month_split = $("#nextMonth").val();
...
$('#month_year').on('change', function(){
    ...
    var user_id = $('#totalUserId').val();
    var designation = $('#totalUserDesignation').val();
    var Commision = $('#totalUserCommision').val();
    const totalAmountMessage = "TotalAmountDate";
    const totalTableMessage = "TotalTableDate";
    // console.log(TotalYear+' '+TotalMonth);
    dataString={
        TotalYear, TotalMonth, totalAmountMessage, user_id, designation, Commision
    }
    dataString2={
        TotalYear, TotalMonth, totalTableMessage, user_id, designation, Commision
    }
    $.ajax({
        type: 'POST',
        url: 'payout/forms/customer_membership/recruitment_payout_month.php',
        data: dataString,
        cache: false,
        success: function(data){
            console.log(data);
...
//get payment details to save in table. --no use--
function paymentId(id, userID, message, Commi, status, col_update){
    ...
    $('#submitPayment').click(function(e){
        ...
        if(paymentMessage.length>0){
            $.ajax({
                type: 'POST',
                url: 'forms/customer_membership/recruitment_payout_paid.php',
                data: dataString,
                cache: false,
                success: function(data){
                    console.log(data);
...
function totalPayoutExel(){
    ...
    var totalUserId = $('#totalUserId').val();
    var totalUserDesignation = $('#totalUserDesignation').val();
    if(totalPayoutDate){
        ...
        //     }
        // });
        window.location.href='payout/forms/customer_membership/download_exel_ca.php?payoutYear='+payoutYear+'&payoutMonth='+payoutMonth+'&payoutmessage='+payoutmessage+'&user_id='+totalUserId+'&designation='+totalUserDesignation;
        // console.log('payoutmessage='+payoutmessage);
        // console.log('payoutYear='+payoutYear);
        // console.log('payoutMonth='+payoutMonth);
        // console.log('totalUserId='+totalUserId);
        // console.log('totalUserDesignation='+totalUserDesignation);
    }else{
...
// All Payout filter option download in exel --no use--
function allPayoutExel(){
    ...
    window.location.href='forms/customer_membership/download_exel_ca?payoutYear='+payoutYear+'&payoutMonth='+payoutMonth+'&payoutmessage='+payoutmessage+'&designation='+designation+'&user_id='+user_id;
}
