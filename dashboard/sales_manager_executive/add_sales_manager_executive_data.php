<?php
    ...
    $message="Added new Sales Executive by Admin";
    $message2="Added new Sales Executive by Admin";
    $fromWhom="1";
    ...
    $sql2= $conn->prepare("SELECT distinct sales_executive_id,SUBSTRING(sales_executive_id,4,6) as tc_id from sales_executive where status='1' OR status='3' order by tc_id DESC limit 1");
    $sql2->execute();
    $sql2->setFetchMode(PDO::FETCH_ASSOC);
    if($sql2->rowCount()>0){
        foreach (($sql2->fetchAll()) as $key3 => $row3) {
            $sales_executive_id=$row3["sales_executive_id"];
        }
        if($sales_executive_id ==''){	
                $uid = 'SME'.$subY.'0001';
        }else{
            $subV=substr($sales_executive_id,3,4);
            if($subV==$subY){
                $sales_executive_id++;
                $sales_executive_id=str_pad($sales_executive_id, 4, '0', STR_PAD_LEFT);
                $uid =$sales_executive_id;
            }else{
                $sales_executive_id++;
                $fid=substr($sales_executive_id,5);
                $newValue = 'SME'.$subY.$fid;
                $Nsales_executive_id=str_pad($newValue, 4, '0', STR_PAD_LEFT);
                $uid =$Nsales_executive_id;
            }
        }
    }else{
        $uid = 'SME'.$subY.'0001';
