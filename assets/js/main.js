...
(function ($) {
    ...
    //         values: [1, 10],
    //         // slide: function (event, ui) {
    //         //     $("#duration").val(ui.values[0] + "N" + " " + ui.values[1] + "N");
    //         // },
    //     });
    //     $("#duration").val($("#slider-range-duration") + "N".slider("values", 0) + " - " + $("#slider-range-duration") + "N".slider("values", 1));
    // });
    $(function () {
        // ✅ PRICE RANGE SLIDER
        $("#slider-range").slider({
        ...
        // Set initial text
        $("#duration-min").text($("#slider-range-duration").slider("values", 0));
        $("#duration-max").text($("#slider-range-duration").slider("values", 1));
    });
    ...
    function destinationResult(item) {
        ...
        return $(`
            <div class="select2-result">
                ...
                <p class="airport-name">${desc}</p>
            </div>
        `);
    }
    
    function destinationSelection(item) {
        if (!item.id) return "Destination";
        let desc = $(item.element).data("description") || "";
        $(".destination-result").html(desc); // Only if this element exists
        return item.text;
    }
