...
$tdsPercentage = 0.02; // example: 5% TDS
...
if ($stmt->rowCount() > 0) {
    ...
    foreach ($rows as $row) {
        ...
        // Amount & message based on user type
        if ($userType == '10') {
        ...
        } elseif ($userType == '16' || $userType == '29') {   //te / franchisee
            $message = $row['te_mess'];
            $amt = $row['te_amt'];
            $status = $row['te_status'];
        } elseif ($userType == '24') {   //bch
            $message = $row['bch_mess'];
            $amt = $row['bch_amt'];
            $status = $row['bch_status'];
        } elseif ($userType == '25' || $userType == '31') {   //bdm / rm
            $message = $row['bdm_mess'];
            $amt = $row['bdm_amt'];
            $status = $row['bdm_status'];
        } elseif ($userType == '26' || $userType == '28' || $userType == '30') {   //bm / mf / sf
            $message = $row['bm_mess'];
            $amt = $row['bm_amt'];
