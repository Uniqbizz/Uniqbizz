...
try {
    ...
    $franchiseRef = isset($postData['id']) ? trim($postData['id']) : '';
    ...
    $checkStmt = $conn->prepare("
        ...
        LIMIT 1
    ");
    $checkStmt->bindValue(':user_ref', $franchiseRef, PDO::PARAM_STR);
    $checkStmt->execute();
    ...
    $idSearch = ctype_digit($search) ? (int)$search : -1;
    $likeSearch = "%$search%";

    $sql = "
        SELECT c.*
        FROM ca_customer c
        INNER JOIN ca_travelagency t 
            ON c.ta_reference_no = t.ca_travelagency_id
        WHERE 
            t.reference_no = :franchise_ref
            AND c.status IN ('1','3')
            AND (
                c.id = :id
                ...
                OR c.reference_no LIKE :search
                OR c.registrant LIKE :search
                OR c.contact_no LIKE :search
                OR c.register_date LIKE :search
            )
    ";

    // Add date range filters if provided
    if (!empty($fromDate) && !empty($toDate)) {
        $sql .= " AND DATE(c.register_date) BETWEEN :from_date AND :to_date";
    } elseif (!empty($fromDate)) {
        $sql .= " AND DATE(c.register_date) >= :from_date";
    } elseif (!empty($toDate)) {
        $sql .= " AND DATE(c.register_date) <= :to_date";
    }

    $stmt = $conn->prepare($sql);
    $stmt->bindValue(':franchise_ref', $franchiseRef, PDO::PARAM_STR);
