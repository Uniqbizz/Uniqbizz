...
try {
    ...
    // 2️⃣ Get Travel Agencies under Franchise (user_type 29 logic)
    $stmtTA = $conn->prepare("
    ...
    // 3️⃣ Build Booking Query (Booked Only)
    $sql = "
    ...
    if (empty($bookings)) {
        ...
        $hasDate   = (!empty($fromDate) || !empty($toDate));
    ...
    foreach ($bookings as $booking) {
        ...
        // Payment validation (must be fully paid like original API)
        $percent_fill = 0;
        $booking_paid_amt = 0;
        $booking_full_amt = 0;
    ...
    echo json_encode([
        ...
        "count" => count($result),
        "bookings" => $result,
        "message" => (!empty($search) || !empty($fromDate)) ? "Filtered results found" : "All booked orders"
    ]);
