...
} else if ($DBtable == 'executive_techno_enterprise') { // 34
    $sql = "SELECT * FROM executive_techno_enterprise WHERE executive_techno_enterprise_id = '" . $id . "' AND status = '1'";
} else if ($DBtable == 'super_techno_enterprise') { // 34
    $sql = "SELECT * FROM super_techno_enterprise WHERE super_techno_enterprise_id = '" . $id . "' AND status = '1'";
}
        ...
        if ($DBtable == 'business_developement_manager' || $DBtable == 'business_chanel_manager' || $DBtable == 'relationship_manager') {
            ...
            if($DBtable == 'zonal_manager'){
                ...
                if($user_type == 32){
                    ...
                    $no_branches=$row['no_of_branches'];
                    $ins_type=$row['types_of_institution'];
                    $ins_phone=$row['country_code'].$row['contact_no'];
                    $ins_pan=$row['institution_pan'];
                    $ins_email=$row['email'];
                    $ins_address=$row['address'];
                    $ins_bank_and_branch_name=$row['bank_and_branch_name'];
                    $ins_incop=$row['certificate_of_incorporation'];
                    $ins_gstin=$row['gstin'];
                    $ins_pan_card=$row['pan_card'];
                    $ins_address_proof=$row['pan_card'];
                    $ins_board_resolution=$row['board_resolution'];
                    $ins_bank_passbook=$row['bank_passbook'];
                    $ins_payment_proof=$row['payment_proof'];
                }else{
                    $firstname = $row['firstname'];
                    $lastname = $row['lastname'];
                }
                if (in_array($user_type, ['34', '35', '36'])) {
                    $sql_doc = "SELECT nominee_name,nominee_relation
                    ...
                    $rown = $stmt_doc->fetch(PDO::FETCH_ASSOC);
                    
                    $nominee_name = $rown['nominee_name'];
                    $nominee_relation = $rown['nominee_relation'];
                ...
                if (!in_array($user_type, ['34', '35', '36','32'])) {$voting_card = $row['voting_card'];}
            }
            $email = $row['email'];
            $date_of_birth = $user_type!=32?$row['date_of_birth']:'';
            $gender = $user_type!=32?$row['gender']:'';
            $country = $row['country'];
            $state = $row['state'];
            $city = $row['city'];
            $address = $row['address'];
            if (in_array($user_type, ['34', '35', '36'])) {
                ...
                $rowp = $stmt_doc->fetch(PDO::FETCH_ASSOC);

                $profile_pic = $rowp['profile_pic'] ?? '';
            }else{
                $profile_pic = $user_type!=32?$row['profile_pic']:'';
            }
            if (in_array($user_type, ['34', '35', '36'])) {
                $sql_doc = "SELECT *
                        FROM documents
                ...
                $documents = $stmt_doc->fetch(PDO::FETCH_ASSOC);
            }else{
                $pan_card = $row['pan_card'];
            }
            if (!in_array($user_type, ['34', '35', '36','32'])) {$aadhar_card = $row['aadhar_card'];}
            ...
            if ($DBtable == 'ca_travelagency' || $DBtable == 'ca_customer' || $DBtable == 'institution_branch_manager') {
                ...
                if (!in_array($user_type, ['34', '35', '36'])) {$bank_passbook = $row['bank_passbook'];}
            }
