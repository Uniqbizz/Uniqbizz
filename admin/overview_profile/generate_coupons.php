...
$regen_check = $_POST['regen_check'] ?? 0;
...
function couponInsert($conn,$customer_id,$payment_label){
...
if($regen_check){
    try {
        ...
        $sql9 = $conn->prepare("
            SELECT * 
            FROM ca_customer 
            WHERE ca_customer_id = :ca_customer_id 
            AND status = '1'
        ");

        $sql9->execute([
            ':ca_customer_id' => $identifier_id
        ]);
        ...
        if ($sql9->rowCount() > 0) {
            ...
            foreach (($sql9->fetchAll()) as $key9 => $row9) {
                $refid = $row9['ta_reference_no'];
            }
        }

        // update regen dates
        $regenDate  = date('Y-m-d H:i:s');
        $expiryDate = date('Y-m-d H:i:s', strtotime($regenDate . ' +10 years'));

        $sql_update_cust = $conn->prepare("
            UPDATE ca_customer 
            SET 
                coupon_regen_check = :coupon_regen_check,
                coupon_regen_date  = :coupon_regen_date
            WHERE ca_customer_id = :id
        ");

        $sql_update_cust->execute([
            ':coupon_regen_check' => $regen_check,
            ':coupon_regen_date'  => $regenDate,
            ':id'                 => $identifier_id
        ]);

        // update coupon expiry
        $sq_update_cust = $conn->prepare("
            UPDATE cu_coupons 
            SET 
                expiry_date = :expiry_date
        ...
        $sq_update_cust->execute([
            ':expiry_date' => $expiryDate,
            ':user_id'     => $identifier_id
        ]);

        // coupon logs
        $message = $identifier_id . " Coupons has been regenerated of " . $payment_label . " type";
        ...
        $stmtcoupon3 = $conn->prepare($sqlcoupon3);

        $stmtcoupon3->execute([
            ':customer_id' => $identifier_id,
            ':regen_date'  =>$regenDate ,
            ':expiry_date' =>$expiryDate,
            ':title'       => 'Coupon Code Regeneration',
            ':message'     => $message,
            ':register_by' => 1,
            ':operation'   => 'Coupon Regeneration'
        ]);
     ...
     try {
        ...
        if ($result1) {
            ...
            //even if its a complementory customer memebers reference payout should go
            ...
            //coupon logs
            $message = $identifier_id . " Coupons has been generated of " . $payment_label . " type";
            $sqlcoupon3 = "INSERT INTO coupon_logs (user_id,title,message, register_by, operation) 
                        VALUES (:user_id,:title ,:message, :message2,:reference_no, :register_by, :from_whom, :operation)";
            $stmtcoupon3 = $conn->prepare($sqlcoupon3);

            $resultcoupon3 = $stmtcoupon3->execute(array(
                ':user_id' => $identifier_id,
                ':title' => 'Coupon Code Generation',
                ':message' => $message,
