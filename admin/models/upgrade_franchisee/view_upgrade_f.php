<?php
    if (substr($userId,0,1) == 'I') {
    ...
    } else if (substr($userId,0,1) == 'I'){
        $sql100= "SELECT amount,reference_no FROM institution WHERE institution_id='".$userId."' and status=1";
        $stmt100 = $conn->prepare($sql100);
        $stmt100->execute();
        $stmt100->setFetchMode(PDO::FETCH_ASSOC);
        if ($stmt100->rowCount() > 0) {
        ...
        $sql101= "SELECT old_investment_amt,new_investment_amt,upgrade_amt as upgrade_amt  FROM institution_upgrade
                            WHERE institution_id='".$userId."' and upgrade_status=1
                            ORDER BY upgrade_approval_date DESC limit 1";
        $stmt101 = $conn->prepare($sql101);
        // print_r($stmt101);
        $stmt101->execute();
