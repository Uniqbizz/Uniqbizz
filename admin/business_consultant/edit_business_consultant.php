...
<?php
        ...
        $stmt = $conn->prepare("SELECT * FROM `business_consultant` where business_consultant_id='".$id."' OR id = '".$id."'");
        $stmt->execute();
        ...
        if($stmt->rowCount()>0){
            foreach (($stmt->fetchAll()) as $key => $row) {
                ...
                }else if($reference_id == "CB"){
                    // CBD name
                    $channel_business_director = $conn->prepare("SELECT firstname, lastname FROM channel_business_director where channel_business_director_id='".$reference_no."'");
                    $channel_business_director ->execute();
                    $channel_business_director ->setFetchMode(PDO::FETCH_ASSOC);
                    if(  $channel_business_director->rowCount()>0 ){
                        $channel_business_director = $channel_business_director->fetch();
                        $reference_no_fname = $channel_business_director['firstname'];
                        $reference_no_lname = $channel_business_director['lastname'];
                    }
                }else if($reference_id == "F"){
                    // franchisee name
                    $franchisees = $conn->prepare("SELECT firstname, lastname FROM franchisee where franchisee_id='".$reference_no."'");
                    $franchisees ->execute();
                    $franchisees ->setFetchMode(PDO::FETCH_ASSOC);
                    if(  $franchisees->rowCount()>0 ){
                        $franchisees = $franchisees->fetch();
                        $reference_no_fname = $franchisees['firstname'];
                        $reference_no_lname = $franchisees['lastname'];
                    }
                } else {
                    // Travel agent name
...
<head>
        ...
        <meta charset="utf-8" />
        <title>Business Consultant Edit | Admin Dashboard </title>
                                            ...
                                            <div class="col-sm-4">
                                                <div class="search-box me-2 mb-2 d-inline-block">
                                                    <div class="position-relative">
                                                        <h4>Edit Business Consultant Form</h4>
                                                    </div>
                                                </div>
                                            </div>
                                            <!-- <div class="col-sm-8">
                                        ...
                                        <div class="row mb-3">
                                            <div class="col-12">
                                                <form>
                                                    ...
                                                    <div>
                                                        <button type="submit" class="btn btn-primary w-md mt-3" id="editBusinessConsultant">Submit</button>
                                                    </div>
