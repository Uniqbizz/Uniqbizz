...
if ($usertype == '24' || $usertype == '25' || $usertype == '31') {
	...
	// Fetch the highest numeric part from all zonal_manager_id, ignoring prefix
	$sql2 = $conn->prepare("
		SELECT zonal_manager_id,
			CAST(RIGHT(zonal_manager_id, 5) AS UNSIGNED) AS numeric_part
		FROM zonal_manager
	...
	$shortName = '';
	if ($row = $sql3->fetch()) {
		$shortName = $row['short_name']; // e.g., MP, GA, AR
	}
	...
	$uid = '';
	if ($row = $sql2->fetch()) {
		$lastNumber = (int)$row['numeric_part'];
	...
	// Generate the ID
	$uid = 'ZM' . $shortName . $subY . $nextNumber;
