...
function pricingFormNext(e) {
	...
	var prime_cust_comm=$('#prime_cust_comm').val();
	...
	if (netPriceAdult == "" || netPriceChild == "" || nGst == "" || ta == "" || company == "" || cus == "" || policy_1 == '' || policy_2 == '' || policy_3 == ''
		|| netPriceAdult == 0 || netPriceChild == 0 || nGst == 0 || ta == 0 || company == 0 || cus == 0 || prime_cust_comm=='' || prime_cust_comm==0) {
		if (netPriceAdult == 0) {
		...
		// } 
		else if (prime_cust_comm == "" || prime_cust_comm == 0) {
			alert("Please enter L1 Customer Amount !");
		}
...
function getFinalPrice() {//changed on 23 Jan 2025 SV
	...
	function extractValueFromLabel(labelId) {
		const text = document.getElementById(labelId).innerText;
		const match = text.match(/Total\s*:\s*(\d+)\)/);
		return match ? parseFloat(match[1]) : 0;
	}

	let TE_mark_up = extractValueFromLabel("ca_label");
	let bm_mark_up = extractValueFromLabel("bm_label");
	...
	TE_mark_up = parseFloat(truncateToTwoDecimals(TE_mark_up));
	bdm_mark_up = parseFloat(truncateToTwoDecimals(bdm_mark_up));
	bcm_mark_up = parseFloat(truncateToTwoDecimals(bcm_mark_up));
	var net_price_adult_without_GST = parseFloat(document.getElementById("netPriceAdult").value) || 0;
	var net_price_child_without_GST = parseFloat(document.getElementById("netPriceChild").value) || 0;
	...
	total_child_price = parseFloat(truncateToTwoDecimals(total_adult_price));

	//without GST price 
	var total_adult_price_with_markup=
		parseFloat(net_price_adult_without_GST)+
		parseFloat(TE_mark_up) +
		parseFloat(bm_mark_up) +
		parseFloat(ta_mark_up) +
		parseFloat(customer_share) +
	...
	var total_child_price_with_markup=
		parseFloat(net_price_child_without_GST)+
		parseFloat(TE_mark_up) +
		parseFloat(bm_mark_up) +
		parseFloat(ta_mark_up) +
		parseFloat(customer_share) +
	...
	total_adult_price_with_markup = truncateToTwoDecimals(total_adult_price_with_markup);
	total_child_price_with_markup = truncateToTwoDecimals(total_child_price_with_markup);
	document.getElementById("mp_adult").value =total_adult_price_with_markup || 0; // new
	document.getElementById("mp_child").value =total_child_price_with_markup || 0;
	//commented on 25 jan by sv
	console.log('netPriceAdult = ' + net_price_adult_without_GST);
	console.log('netPriceChild = ' + net_price_child_without_GST);
	console.log('netPriceAdult = ' + net_price_adult_without_GST);
	console.log('netPriceChild = ' + net_price_child_without_GST);
...
function finalfill() {//added on 23 Jan 2025 SV
	...
	let prime_customer_share = truncateToTwoDecimals(parseFloat($('#prime_cust_comm').val()));//take it from an input field
	let L1_customer_share = truncateToTwoDecimals(parseFloat(prime_customer_share * 0.5));
	let L2_customer_share = truncateToTwoDecimals(parseFloat(L1_customer_share * 0.5));
	
	//code change by savio 24-06-25
	customer_share = prime_customer_share + L1_customer_share + L2_customer_share;
	//set the customer share
...
// $('#prime_cust_comm').on('input',function(){
...
function getMarkUpDistribution(payoutData) {// //changed on 23 Jan 2025 SV
	...
	//customer comission structure by savio 24-06-25
	let prime_customer_share = truncateToTwoDecimals(parseFloat($('#prime_cust_comm').val()));//take it from an input field
	let L1_customer_share = truncateToTwoDecimals(parseFloat(prime_customer_share * 0.5));
	let L2_customer_share = truncateToTwoDecimals(parseFloat(L1_customer_share * 0.5));
	//code change by savio 24-06-25
...
function submit_form_data(e) {
	...
	if (image_data == "") {
		...
		var total_adult_price_with_markup=$('#mp_adult').val();
		var total_child_price_with_markup=$('#mp_child').val() || 0;
		...
		var formdata = {
			...
			total_child_price_with_markup:total_child_price_with_markup,
			total_adult_price_with_markup:total_adult_price_with_markup
		};
