...
var cap_month = document.getElementById("month_year"); // change placeholder input box to actual date input
...
$('#designation').on('change', function () {
    ...
    $.ajax({
        type: 'POST',
        url: '../../controllers/payout/forms/customer_recruitment_payout/customer_membership_payout_filter.php',
        data: 'designation=' + designation,
        success: function (data) {
            // console.log(data);
...
$('#user_id_namePrevious').on('change', function () {
    ...
    $.ajax({
        type: 'POST',
        url: '../../controllers/payout/forms/customer_recruitment_payout/customer_membership_payout_filter_amt_user.php',
        data: dataString,
        cache: false,
        success: function (data) {
            $('#prevDiv').html(data);
...
$('#month_year').on('change', function () {
    ...
    $.ajax({
        type: 'POST',
        url: '../../controllers/payout/forms/customer_recruitment_payout/customer_membership_payout_month.php',
        data: dataString,
        cache: false,
        success: function (data) {
            console.log(data);
...
function paymentId(id, userID, message, Commi, status, col_update) {
    ...
    $('#submitPayment').click(function (e) {
        ...
        if (paymentMessage.length > 0) {
            $.ajax({
                type: 'POST',
                url: '../../controllers/payout/forms/customer_recruitment_payout/customer_membership_payout_paid.php',
                data: dataString,
                cache: false,
                success: function (data) {
                    console.log(data);
...
function totalPayoutExel() {
    ...
    if (totalPayoutDate) {
        ...
        //     }
        // });
        window.location.href = '../../controllers/payout/forms/customer_recruitment_payout/download_exel_cu.php?payoutYear=' + payoutYear + '&payoutMonth=' + payoutMonth + '&payoutmessage=' + payoutmessage;
    } else {
...
// All Payout filter option download in exel 06-08-2025
function allPayoutExel() {
    var designation = $('#designation').val() || 'null';
    var user_id = $('#user_id_name').val() || 'null';
    ...
    if (typeof date === 'string' && date.includes('-')) {
        ...
        if (myArray.length >= 2) {
            payoutYear = myArray[0] || 'null';
            payoutMonth = myArray[1] || 'null';
        }
    }
    // console.log(designation + user_id + date + payoutmessage);
    window.location.href = '../../controllers/payout/forms/customer_recruitment_payout/download_exel_cu.php?payoutYear=' + payoutYear + '&payoutMonth=' + payoutMonth + '&payoutmessage=' + payoutmessage + '&designation=' + designation + '&user_id=' + user_id;
}


// **** customer_recruitment_payout Javascript End ****
