...
$uname= $_POST["uname"];
	...
	$sql9= $conn->prepare("SELECT * from business_operation_executive where id='".$id."' and status='2'");
	$sql9->execute();
    ...
    $sql2= $conn->prepare("SELECT distinct business_operation_executive_id,SUBSTRING(business_operation_executive_id,4,6) as tc_id from business_operation_executive where status='1' OR status='3' order by tc_id DESC limit 1");
    $sql2->execute();
    $sql2->setFetchMode(PDO::FETCH_ASSOC);
    if($sql2->rowCount()>0){
        foreach (($sql2->fetchAll()) as $key3 => $row3) {
            $business_operation_executive_id=$row3["business_operation_executive_id"];
        }
        if($business_operation_executive_id ==''){	
                $uid = 'BOE'.$subY.'0001';
        }else{
            $subV=substr($business_operation_executive_id,3,4);
            if($subV==$subY){
                $business_operation_executive_id++;
                $business_operation_executive_id=str_pad($business_operation_executive_id, 4, '0', STR_PAD_LEFT);
                $uid =$business_operation_executive_id;
            }else{
                $business_operation_executive_id++;
                $fid=substr($business_operation_executive_id,5);
                $newValue = 'BOE'.$subY.$fid;
                $Nbusiness_operation_executive_id=str_pad($newValue, 4, '0', STR_PAD_LEFT);
                $uid =$Nbusiness_operation_executive_id;
            }
        }
    }else{
        $uid = 'BOE'.$subY.'0001';
...
//log file
$title="Confirm Business Operation Executive";
...
$sql1 = "UPDATE business_operation_executive SET status=:status,business_operation_executive_id=:business_operation_executive_id WHERE id=:id";
$stmt = $conn->prepare($sql1);
$result=  $stmt->execute(array(
	':status' => $status,
	':business_operation_executive_id' => $uid,
	// ':deleted_date' => $today,
	':id' => $id		
));
...
if ($result) {
	...
	if($result2){
		...
		if($result3){
			...
			// $message_bc = "BC - ".$Bc_name." ".$Bc_id." earned ".$bcCommiAmt."/- on recruting Corporate Agency. Name of the Corporate Agency - " .$name." ".$uid. ". Recruitment Fee - ".$amount." . ";
