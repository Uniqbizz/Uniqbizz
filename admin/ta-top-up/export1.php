...
foreach ($ta_ids as $ta_id) {
    $stmt = $conn->prepare("SELECT ta_id, ta_fname, ta_lname, SUM(top_up_amt) AS total_amt 
    ...
    if ($referral) {
        $total_amt=$total_amt+$referral['total_amt'];
    }
}
fputcsv($output,['','Total Amount ',number_format($total_amt,2)]);
