<?php
	...
	if ($user_type_id == '34') { //Executive Techno Enterprise
		$sql9 = $conn->prepare("SELECT * from executive_techno_enterprise where id='" . $id . "' and status='2'");
		$sql9->execute();
		...
		$sql10 = $conn->prepare("SELECT * FROM executive_techno_enterprise WHERE executive_techno_enterprise_id = '" . $reference_no . "' AND user_type = '36' AND status = '1' ");
		$sql10->execute();
		$sql10->setFetchMode(PDO::FETCH_ASSOC);
		if ($sql10->rowCount() > 0) {
			foreach (($sql10->fetchAll()) as $key10 => $row10) {
				$bdm_id = $row10['executive_techno_enterprise_id'];
		...
		$sql2 = $conn->prepare("
			SELECT executive_techno_enterprise_id,
				CAST(RIGHT(executive_techno_enterprise_id, 5) AS UNSIGNED) AS numeric_part
		...
		$uid = 'ETE' . $subY . $nextNumber;

		//log file
		$title = "Confirm Executive Techno Enterprise";
		$message = $uid . " has been approved";
		$message2 = $uid . " has been approved";
		$fromWhom = "1";
		$operation = "Confirm";

		$sql1 = "UPDATE executive_techno_enterprise SET status=:status,executive_techno_enterprise_id=:executive_techno_enterprise_id,register_date=:register_date WHERE id=:id";
		$stmt = $conn->prepare($sql1);
		$resultSte =  $stmt->execute(array(
			':status' => $status,
			':executive_techno_enterprise_id' => $uid,
		...
		if($resultSte){
			// enter Executive Techno Enterprise uniq id in all tables mentioned below
			$tables = [
			...
			if ($result2) {
				...
				if ($result3) {
					...
					if ($result4) {
						...
						$userTypeName = 'Executive Techno Enterprise';
