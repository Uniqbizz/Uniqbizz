<?php
    ...
    if ($editfor == 'pending') {
        ...
        $message = "Updated Sponsor Franchisee details from the pending list";
    } else {
        $identifier_column = 'sponsor_franchisee_id';
        $message = "{$identifier_id} Details has been updated from the registered list";
    }

    /* FETCH PREVIOUS DATA */

    $stmt = $conn->prepare("SELECT * FROM sponsor_franchisee WHERE $identifier_column = :id");
    ...
    /* UPDATE SPONSOR FRANCHISEE */

    $sql = "UPDATE sponsor_franchisee SET
