    ...
    if (!empty($designation)) {
        if ($designation == '26') { // BM
            if (!empty($userId)) {
                ...
                // include TCs from TEs under BMs
            ...
            if (!empty($userId)) {
                ...
                // include TCs from TEs under BMs
                $stmt = $conn->prepare("SELECT sub_franchisee_id FROM sub_franchisee WHERE reference_no LIKE 'BM%' AND (status = '1' OR status = '3')");
                $stmt->execute();
                $teList = $stmt->fetchAll(PDO::FETCH_COLUMN);
        ...
        } elseif ($designation == '30') { // sf
            if (!empty($userId)) {
                // include TCs from Fs under SFs
                $stmt = $conn->prepare("SELECT sub_franchisee_id FROM sub_franchisee WHERE reference_no LIKE 'BM%' AND (status = '1' OR status = '3')");
                $stmt->execute();
                $teList = $stmt->fetchAll(PDO::FETCH_COLUMN);
