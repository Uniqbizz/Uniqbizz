...
if($totalAmountMessage){
    $stmt = " SELECT SUM(amount) as TotalPayout FROM cbd_payout WHERE YEAR(created_date) = '".$TotalYear."' AND MONTH(created_date) = '".$TotalMonth."' AND status = '1' ";
...
if($totalTableMessage){
    echo '<table class="table table-hover table-responsive" id="totalPayoutTable">
        ...
        <tbody >';
            ...
            $model2 = "SELECT * FROM cbd_payout WHERE YEAR(created_date) = '".$TotalYear."' AND MONTH(created_date) = '".$TotalMonth."' AND status = '1'";
