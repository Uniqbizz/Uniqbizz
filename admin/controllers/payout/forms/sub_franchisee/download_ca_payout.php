...
if($user_id == "MF"){
    $bcNames = $conn -> prepare("SELECT * FROM master_franchisee WHERE master_franchisee_id = '".$userId."' AND status = 1");
    $bcNames -> execute();
    ...
    $bcNames = $conn -> prepare("SELECT * FROM sponsor_franchisee WHERE sponsor_franchisee_id = '".$userId."' AND status = 1");
    $bcNames -> execute();
    $bcNames -> setFetchMode(PDO::FETCH_ASSOC);
    if($bcNames -> rowCount()>0){
        foreach(($bcNames -> fetchAll()) as $key => $row){
    ...
    $bcNames = $conn -> prepare("SELECT * FROM zonal_manager WHERE zonal_manager_id = '".$userId."' AND status = 1");
    $bcNames -> execute();
    $bcNames -> setFetchMode(PDO::FETCH_ASSOC);
    if($bcNames -> rowCount()>0){
        foreach(($bcNames -> fetchAll()) as $key => $row){
    ...
    <body>
        <div class="background" >
            <div class="container cont-btn d-flex justify-content-around pt-3 pb-4">
                <a href="../../../../views/payout/sub_franchisee_payout.php" class="go-back"> Go Back</a>
                    ...
                    <div class="col-md-12 col-sm-12 col-12 ps-3 pe-3" >
                        <div class="row">
                                ...
                                <tbody>
                                    <tr class="row">
                                        <td class="col-md-7 col-sm-7 left pt-3">
                                            ...
                                            <h6 style="padding:2px 10px; font-weight: 700;">User ID : <?php echo $userId; ?></h6>
                            ...
                            <div class="col-md-12 col-sm-12" >
                                <h5  style="padding: 10px 5px;  margin:0px; font-weight: 700; ">Franchisee Payout</h5>
