...
$bc = $_GET['bc'];
$bc_str=substr($bc,0,2);
$ca = $_GET['ca'];
$ca_str=substr($ca,0,1) == 'F'?substr($ca,0,1):substr($ca,0,2);
$ta_ca = $_GET['ta_ca'];
$dateCA = $_GET['date'];
...
if ($bc_str == 'MF') {
    $bcNames = $conn -> prepare("SELECT * FROM master_franchisee WHERE master_franchisee_id = '".$bc."' AND status = 1");
    $bcNames -> execute();
    $bcNames -> setFetchMode(PDO::FETCH_ASSOC);
    if($bcNames -> rowCount()>0){
        foreach(($bcNames -> fetchAll()) as $key => $row){
...
if ($bc_str == 'SF') {
    $bcNames = $conn -> prepare("SELECT * FROM sponsor_franchisee WHERE sponsor_franchisee_id = '".$bc."' AND status = 1");
    $bcNames -> execute();
    $bcNames -> setFetchMode(PDO::FETCH_ASSOC);
    if($bcNames -> rowCount()>0){
        foreach(($bcNames -> fetchAll()) as $key => $row){
...
if ($ca_str == "TE" || $ca_str == "CA") {
    ...
    if($caNames -> rowCount()>0){
        foreach(($caNames -> fetchAll()) as $key => $row){
            ...
            $designation1 = "Techno Enterprise";
        }
    } 
}
if ($ca_str == "F") {
    ...
    <body>
        <div class="background" >
            <div class="container cont-btn d-flex justify-content-around pt-3 pb-4">
                <a href="../../../../views/payout/recruitment_payout.php" class="go-back"> Go Back</a>
                    ...
                    <div class="col-md-12 col-sm-12 col-12 ps-3 pe-3" >
                        <div class="row">
                            ...
                            <div class="col-md-12 col-sm-12" style="" >
                                <h5  style="padding: 10px 5px;  margin:0px; font-weight: 700; "><?= $designation1 ?> Payout</h5>
