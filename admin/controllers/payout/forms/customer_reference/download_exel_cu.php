...
if($payoutmessage == 'PreviousPayout'){
    ...
    $stmt2 = "SELECT * FROM customer_reference_payout 
              WHERE MONTH(created_date) = '".$payoutMonth."' AND YEAR(created_date) = '".$payoutYear."'";
    ...
    if($stmt2 -> rowCount()>0){
        ...
        <table border="1" style="text-align:center">
            ...
            foreach($stmt2->fetchAll() as $key => $row2){
                ...
                // get the commission amount of CU's
                ...
                $sql2= $conn->prepare("SELECT firstname,lastname FROM `ca_customer` where ca_customer_id='".$row2['customer_id']."'");
                $sql2->execute();
                ...
                $sql3= $conn->prepare("SELECT firstname,lastname FROM `ca_customer` where ca_customer_id='".$row2['refered_customer_id']."'");
                $sql3->execute();
                $sql3->setFetchMode(PDO::FETCH_ASSOC);
                if($sql3->rowCount()>0){
                    foreach (($sql3->fetchAll()) as $key => $row3) {
                        $ref_ca_name = $row3['firstname']. ' ' .$row3['lastname'];
                    }
                } 
