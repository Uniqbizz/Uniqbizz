...
if($totalAmountMessage){
    $stmt = " SELECT SUM(comm_amt) as TotalPayout FROM ca_payout WHERE YEAR(created_date) = '".$TotalYear."' AND MONTH(created_date) = '".$TotalMonth."' AND status = '1' ";
...
if($totalTableMessage){
    echo '<table class="table table-hover table-responsive" id="totalPayoutTable">
        ...
        <tbody >';
            ...
            $model2 = "SELECT * FROM ca_payout WHERE YEAR(created_date) = '".$TotalYear."' AND MONTH(created_date) = '".$TotalMonth."' AND status = '1'";
