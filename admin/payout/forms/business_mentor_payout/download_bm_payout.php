...
if($bdm == "Not Applicable"){
    $empName = 'BMD Not Applicable';
}else{
    $empNames = $conn -> prepare("SELECT * FROM employees WHERE employee_id = '".$bdm."' AND user_type = '25' AND status = 1");
    $empNames -> execute();
    $empNames -> setFetchMode(PDO::FETCH_ASSOC);
    if($empNames -> rowCount()>0){
        foreach(($empNames -> fetchAll()) as $key => $row){
            $empName = $row['name'];
            // $bclastname = $row['lastname']; no last name for employees
        }
    }  
}

$bmNames = $conn -> prepare("SELECT * FROM business_mentor WHERE business_mentor_id = '".$bm."' AND status = 1");
$bmNames -> execute();
$bmNames -> setFetchMode(PDO::FETCH_ASSOC);
if($bmNames -> rowCount()>0){
    foreach(($bmNames -> fetchAll()) as $key => $row){
        $bmfirstname = $row['firstname'];
        $bmlastname = $row['lastname'];
    }
} 
    ...
    <body>
        <div class="background" >
            <div class="container cont-btn d-flex justify-content-around pt-3 pb-4">
                <a href="../../business_mentor_payout.php" class="go-back"> Go Back</a>
                    ...
                    <div class="col-md-12 col-sm-12 col-12 ps-3 pe-3" style=" ">
                        <div class="row">
                                ...
                                <tbody>
                                    <tr class="row">
                                        <td class="col-md-7 col-sm-7 left pt-3">
                                            <h6 style="padding:2px 10px; font-weight: 700;">Name : <?php echo $empName ; ?></h6>
                                            <h6 style="padding:2px 10px; font-weight: 700;">User ID : <?php echo $bdm; ?></h6>
                                            <h6 style="padding:2px 10px; font-weight: 700;">Month : <?php echo $date; ?></h6>
                                        </td>
                                        <td class="col-md-5 col-sm-5 pt-3">
                                            <h6 style="padding:2px 0; font-weight: 700;">Designation : BDM</h6>
                            ...
                            <div class="col-md-12 col-sm-12" style="" >
                                <h5  style="padding: 10px 5px;  margin:0px; font-weight: 700; ">Business Mentor Payout</h5>
