<?php
	...
	$sql2= $conn->prepare("SELECT distinct branch_manager_id,SUBSTRING(branch_manager_id,5,6) as tc_id from branch_manager where status='1' OR status='3' order by tc_id DESC limit 1");
	$sql2->execute();
	...
	if($sql2->rowCount()>0){
		...
		if($branch_manager_id ==''){	
			...
			$subV=substr($branch_manager_id,4,4);
			if($subV==$subY){
				$branch_manager_id++;
				$branch_manager_id=str_pad($branch_manager_id, 4, '0', STR_PAD_LEFT);
				$uid =$branch_manager_id;
			}else{
				$branch_manager_id++;
				$fid=substr($branch_manager_id,6);
				$newValue = 'BMGA'.$subY.$fid;
				$Nbranch_manager_id=str_pad($newValue, 4, '0', STR_PAD_LEFT);
				$uid =$Nbranch_manager_id;
			}
		}
	}else{
		$uid = 'BMGA'.$subY.'0001';
