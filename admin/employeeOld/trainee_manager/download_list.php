...
$output="";
    ...
    $stmt = $conn->prepare("SELECT * FROM `trainee_manager` where status='".$status."'");
    $stmt->execute();
    ...
    if($stmt->rowCount()>0){
    	...
    	$output .= '<h2 style="text-align:center">Trainee Manager '.$name.' List</h2>
        <table border="1" style="text-align:center">
    	<tr>';
            if($name == 'Pending'){
                $output .= '<th>S.No</th>';
            }else if($name == 'Registered'){
                $output .='<th>Trainee Manager ID</th>';
            }
    	$output .= '	
    		<th>Name</th>
    		<th>Email</th>
    ...
    foreach (($stmt->fetchAll()) as $key => $row) {
    	...
    	$output .= '<tr>';
            if($name == 'Pending'){
                ...
                $output .='<td>'.$row['trainee_manager_id'].'</td>';
            }
            $output .= '
    ...
    if($name == 'Pending'){
        header("Content-Disposition: attachment;filename=Pending_trainee_manager_List.xls");
    }else if($name == 'Registered'){
        header("Content-Disposition: attachment;filename=Registered_trainee_manager_List.xls");
    }
