<?php
    ...
    $sql2= $conn->prepare("SELECT distinct regional_manager_id,SUBSTRING(regional_manager_id,5,6) as tc_id from regional_manager where status='1' OR status='3' order by tc_id DESC limit 1");
	$sql2->execute();
	...
	if($sql2->rowCount()>0){
		...
		if($regional_manager_id ==''){	
			...
			$subV=substr($regional_manager_id,4,4);
			if($subV==$subY){
				// ''.$ssd
				$regional_manager_id++;
				$regional_manager_id=str_pad($regional_manager_id, 4, '0', STR_PAD_LEFT);
				$uid =$regional_manager_id;
			}else{

				$regional_manager_id++;
				$fid=substr($regional_manager_id,6);
				$newValue = 'RMGA'.$subY.$fid;
				$Nregional_manager_id=str_pad($newValue, 4, '0', STR_PAD_LEFT);
				$uid =$Nregional_manager_id;
			}
		}
	}else{
		$uid = 'RMGA'.$subY.'0001';
	...
	$sql= "INSERT INTO regional_manager (regional_manager_id, firstname,lastname, email, country_code, contact_no , date_of_birth,age,gender,country,state,city,pincode,address,profile_pic,pan_card,aadhar_card,voting_card,bank_passbook,user_type,branch,zone,region,registrant,reference_no,register_by,status) VALUES (:regional_manager_id,:firstname ,:lastname, :email, :country_code, :phone_no, :bdate, :age, :gender , :country, :state, :city, :pincode,:address,:profile_pic ,:pan_card,:aadhar_card,:voting_card,:passbook,  :user_type,:branch_name,  :zone_name, :region_name, :registrant, :reference_no, :register_by,:status)";
