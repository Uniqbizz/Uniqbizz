<?php
	...
	$title="Zonal Manager";
	$message="Added New Zonal Manager by ".$rm_id;
	$message2="Added New Zonal Manager by ".$rm_id;
	$fromWhom="1";
	// $register_by="5";
	...
	$sql2= $conn->prepare("SELECT distinct zonal_manager_id,SUBSTRING(zonal_manager_id,5,6) as tc_id from zonal_manager where status='1' OR status='3' order by tc_id DESC limit 1");
	$sql2->execute();
	$sql2->setFetchMode(PDO::FETCH_ASSOC);
	if($sql2->rowCount()>0){
		foreach (($sql2->fetchAll()) as $key3 => $row3) {
			$zonal_manager_id=$row3["zonal_manager_id"];
		}
		if($zonal_manager_id ==''){	
				$uid = 'ZLMG'.$subY.'0001';
		}else{
			$subV=substr($zonal_manager_id,4,4);
			if($subV==$subY){
				$zonal_manager_id++;
				$zonal_manager_id=str_pad($zonal_manager_id, 4, '0', STR_PAD_LEFT);
				$uid =$zonal_manager_id;
			}else{

				$zonal_manager_id++;
				$fid=substr($zonal_manager_id,6);
				$newValue = 'ZLMG'.$subY.$fid;
				$Nzonal_manager_id=str_pad($newValue, 4, '0', STR_PAD_LEFT);
				$uid =$Nzonal_manager_id;
			}
		}
	}else{
		$uid = 'ZLMG'.$subY.'0001';
	...
	$sql= "INSERT INTO zonal_manager (zonal_manager_id,firstname,lastname, email, country_code, contact_no , date_of_birth,age,gender,country,state,city,pincode,address,profile_pic,pan_card,aadhar_card,voting_card,bank_passbook,user_type,branch,zone,region,registrant,reference_no,register_by, status) VALUES (:zonal_manager_id,:firstname ,:lastname, :email, :country_code, :phone_no, :bdate, :age, :gender , :country, :state, :city, :pincode,:address,:profile_pic ,:pan_card,:aadhar_card,:voting_card,:passbook,  :user_type,:branch_name,  :zone_name, :region_name,:registrant,  :rm_id, :register_by, :status)";
	$stmt3 =$conn->prepare($sql);

	$result=$stmt3->execute(array(
		':zonal_manager_id' => $uid,
		':firstname' => $firstname, 
		':lastname' => $lastname, 
		':email' => $email,
