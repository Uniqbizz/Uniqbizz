<?php 
	...
	$status= '1';
	$user_type_id= '11';
	$user_type_id= $utype == 'tc' ? '11' : ($utype == 'ibr' ? '33' :'');
	...
	if ($user_type_id == '11') { //TC
				...
				if($result3){
					...
					if($result4){
						...
						if($result5){
							...
							$userTypeName = 'Travel Consultant';
	...
	} else if($user_type_id == '33') { //IBR
		$sql9= $conn->prepare("SELECT * from institution_branch_manager where id='".$id."' and status='2'");
		$sql9->execute();
		$sql9->setFetchMode(PDO::FETCH_ASSOC);
		if($sql9->rowCount()>0){
			foreach (($sql9->fetchAll()) as $key9 => $row9) {
		...
		$sql3->execute();
		$shortName = $sql3->fetchColumn();

		// Prefix (without running number)
		$prefix = 'IBR' . $shortName . $subY;

		// Get last record for same prefix
		$sql2 = $conn->prepare("
			SELECT institution_branch_manager_id 
			FROM institution_branch_manager 
			WHERE institution_branch_manager_id LIKE :prefix 
			ORDER BY institution_branch_manager_id DESC 
			LIMIT 1
		");

		$likePrefix = $prefix . '%';
		$sql2->bindParam(':prefix', $likePrefix);
		$sql2->execute();

		$lastId = $sql2->fetchColumn();
		...
		// Pad to 3 digits → 001, 002, 003...
		$runningNumber = str_pad($newNumber, 3, '0', STR_PAD_LEFT);

		// Final UID
		$uid = $prefix . $runningNumber;
		...
		$sql1 = "UPDATE institution_branch_manager SET status=:status,institution_branch_manager_id=:institution_branch_manager_id,register_date=:register_date WHERE id=:id";
		$stmt = $conn->prepare($sql1);
		$result=  $stmt->execute(array(
			':status' => $status,
			':institution_branch_manager_id' => $uid,
			':register_date' => $register_Date,
			// ':deleted_date' => $today,
			':id' => $id		
		));
		...
		if ($result) {
			...
			if($result2){
				...
				if($result3){
					// no payout on recruiting institution branch manager
					...
					if($result4){
						...
						if($result5){
							...
							$to = $uname; //$uname contains email of user which is now registering 
							$subject = 'Login Credentials - Bizzmirth Holidays Pvt ltd';
							$userTypeName = 'Institution Branch Manager';
