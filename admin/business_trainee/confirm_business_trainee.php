...
$sql9= $conn->prepare("SELECT * from business_trainee where id='".$id."' and status='2'");
$sql9->execute();
...
$sql2= $conn->prepare("SELECT distinct business_trainee_id,SUBSTRING(business_trainee_id,3,6) as tc_id from business_trainee where status='1' OR status='3' order by tc_id DESC limit 1");

$sql2->execute();
$sql2->setFetchMode(PDO::FETCH_ASSOC);
if($sql2->rowCount()>0){
	foreach (($sql2->fetchAll()) as $key3 => $row3) {
		 $business_trainee_id=$row3["business_trainee_id"];
	}
	if($business_trainee_id ==''){	
			$uid = 'BT'.$subY.'0001';
	}else{

		$subV=substr($business_trainee_id,2,4);
		if($subV==$subY){
			// ''.$ssd
			$business_trainee_id++;
			  $business_trainee_id=str_pad($business_trainee_id, 4, '0', STR_PAD_LEFT);
			  $uid =$business_trainee_id;
		}else{

			$business_trainee_id++;
			$fid=substr($business_trainee_id,4);
			$newValue = 'BT'.$subY.$fid;
			...
			// }
			  ...
			  $Nbusiness_trainee_id=str_pad($newValue, 4, '0', STR_PAD_LEFT);
			  $uid =$Nbusiness_trainee_id;
		}
	}
...
//log file
$title="Confirm Business Trainee";
$message=$uid." has been approved";
$message2=$uid." has been approved";
$fromWhom="1";

	$sql1 = "UPDATE business_trainee SET status=:status,business_trainee_id=:business_trainee_id WHERE id=:id";
	$stmt = $conn->prepare($sql1);
	$result=  $stmt->execute(array(
		':status' => $status,
		':business_trainee_id' => $uid,
		// ':deleted_date' => $today,
		':id' => $id		
	));
