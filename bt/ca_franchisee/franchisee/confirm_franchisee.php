...
$sql9= $conn->prepare("SELECT * from ca_franchisee where id='".$id."' and status='2'");
$sql9->execute();
...
if($sql9->rowCount()>0){
    ...
    $sql2= $conn->prepare("SELECT distinct ca_franchisee_id,SUBSTRING(ca_franchisee_id,4,6) as tc_id from ca_franchisee where status='1' OR status='3' order by tc_id DESC limit 1");
    $sql2->execute();
    $sql2->setFetchMode(PDO::FETCH_ASSOC);
    if($sql2->rowCount()>0){
        foreach (($sql2->fetchAll()) as $key3 => $row3) {
            $ca_franchisee_id=$row3["ca_franchisee_id"];
        }
        if($ca_franchisee_id ==''){	
                $uid = 'CAF'.$subY.'0001';
        }else{
            $subV=substr($ca_franchisee_id,3,4);
            if($subV==$subY){
                $ca_franchisee_id++;
                $ca_franchisee_id=str_pad($ca_franchisee_id, 4, '0', STR_PAD_LEFT);
                $uid =$ca_franchisee_id;
            }else{
                $ca_franchisee_id++;
                $fid=substr($ca_franchisee_id,5);
                $newValue = 'CAF'.$subY.$fid;
                $Nca_franchisee_id=str_pad($newValue, 4, '0', STR_PAD_LEFT);
                $uid =$Nca_franchisee_id;
            }
        }
    }else{
        $uid = 'CAF'.$subY.'0001';
    ...
    //log file
    $title="Confirm CA Franchisee";
    $message=$uid." has been approved";
    $message2=$uid." has been approved";
    $fromWhom="1";

	$sql1 = "UPDATE ca_franchisee SET status=:status,ca_franchisee_id=:ca_franchisee_id WHERE id=:id";
	$stmt = $conn->prepare($sql1);
	$result=  $stmt->execute(array(
		':status' => $status,
		':ca_franchisee_id' => $uid,
		// ':deleted_date' => $today,
		':id' => $id		
	));
