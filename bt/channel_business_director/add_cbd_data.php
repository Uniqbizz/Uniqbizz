<?php
    ...
    $message="Added new Channel Business Director by admin";
    $message2="Added new Channel Business Director by admin";
    $fromWhom="1";
    ...
    //select Uniqe user from exesiting users 
    $sql2= $conn->prepare("SELECT distinct channel_business_director_id,SUBSTRING(channel_business_director_id,4,6) as cbd_id from channel_business_director where status='1' OR status='3' order by cbd_id DESC limit 1");
    $sql2->execute();
    $sql2->setFetchMode(PDO::FETCH_ASSOC);
    if($sql2->rowCount()>0){
        foreach (($sql2->fetchAll()) as $key3 => $row3) {
            $channel_business_director_id=$row3["channel_business_director_id"];
        }
        if($channel_business_director_id ==''){	
                $uid = 'CBD'.$subY.'0001';
        }else{
            $subV=substr($channel_business_director_id,3,4);
            if($subV==$subY){
                $channel_business_director_id++;
                $channel_business_director_id=str_pad($channel_business_director_id, 4, '0', STR_PAD_LEFT);
                $uid =$channel_business_director_id;
            }else{
                $channel_business_director_id++;
                $fid=substr($channel_business_director_id,5);
                $newValue = 'CBD'.$subY.$fid;
                $Nchannel_business_director_id=str_pad($newValue, 4, '0', STR_PAD_LEFT);
                $uid =$Nchannel_business_director_id;
            }
        }
    }else{
        $uid = 'CBD'.$subY.'0001';
    ...
    if ($result) {
        ...
        if($result2){
            ...
            $sql2= "INSERT INTO logs (user_id,title,message,message2,reference_no, register_by, from_whom) VALUES (:user_id,:title ,:message, :message2, :reference_no, :register_by, :from_whom)";
