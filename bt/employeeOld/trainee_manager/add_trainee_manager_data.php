<?php
	...
	$message="Added New Trainee Manager";
	$message2="Added New Trainee Manager";
	$fromWhom="1";
	// $register_by="5";
	...
	$sql2= $conn->prepare("SELECT distinct trainee_manager_id,SUBSTRING(trainee_manager_id,5,6) as tc_id from trainee_manager where status='1' OR status='3' order by tc_id DESC limit 1");
	$sql2->execute();
	$sql2->setFetchMode(PDO::FETCH_ASSOC);
	if($sql2->rowCount()>0){
		foreach (($sql2->fetchAll()) as $key3 => $row3) {
			$trainee_manager_id=$row3["trainee_manager_id"];
		}
		if($trainee_manager_id ==''){	
				$uid = 'TRMG'.$subY.'0001';
		}else{

			$subV=substr($trainee_manager_id,4,4);
			if($subV==$subY){
				$trainee_manager_id++;
				$trainee_manager_id=str_pad($trainee_manager_id, 4, '0', STR_PAD_LEFT);
				$uid =$trainee_manager_id;
			}else{
				$trainee_manager_id++;
				$fid=substr($trainee_manager_id,6);
				$newValue = 'TRMG'.$subY.$fid;
				$Ntrainee_manager_id=str_pad($newValue, 4, '0', STR_PAD_LEFT);
				$uid =$Ntrainee_manager_id;
			}
		}
	}else{
		$uid = 'TRMG'.$subY.'0001';
