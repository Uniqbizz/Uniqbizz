<?php
	...
	$sql2= $conn->prepare("SELECT distinct sales_manager_id,SUBSTRING(sales_manager_id,6,6) as tc_id from sales_manager where status='1' OR status='3' order by tc_id DESC limit 1");
	$sql2->execute();
	...
	if($sql2->rowCount()>0){
		...
		if($sales_manager_id ==''){	
			...
			$subV=substr($sales_manager_id,5,4);
			if($subV==$subY){
				$sales_manager_id++;
				$sales_manager_id=str_pad($sales_manager_id, 4, '0', STR_PAD_LEFT);
				$uid =$sales_manager_id;
			}else{
				$sales_manager_id++;
				$fid=substr($sales_manager_id,7);
				$newValue = 'FLMGA'.$subY.$fid;
				$Nsales_manager_id=str_pad($newValue, 4, '0', STR_PAD_LEFT);
				$uid =$Nsales_manager_id;
			}
		}
	}else{
		$uid = 'FLMGA'.$subY.'0001';
	...
	$sql= "INSERT INTO sales_manager (sales_manager_id,firstname,lastname, email, country_code, contact_no , date_of_birth,age,gender,country,state,city,pincode,address,profile_pic, pan_card,aadhar_card,voting_card,bank_passbook,user_type,branch,zone,region,registrant,reference_no,register_by, status) VALUES (:sales_manager_id,:firstname ,:lastname, :email, :country_code, :phone_no, :bdate, :age, :gender , :country, :state, :city, :pincode,:address,:profile_pic ,:pan_card,:aadhar_card,:voting_card,:passbook,  :user_type,:branch_name,  :zone_name, :region_name,:registrant,  :bm_id, :register_by,:status)";
