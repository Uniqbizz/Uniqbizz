<?php
    ...
    if ($user_id == '0') {
        $sql = $conn->prepare("SELECT travel_agent_id, MONTH(register_date) AS start_month, YEAR(register_date) AS start_year FROM travel_agent where status='1' ");
    } else {
        $sql = $conn->prepare("SELECT travel_agent_id, MONTH(register_date) AS start_month, YEAR(register_date) AS start_year 
                                FROM travel_agent where reference_no='".$user_id."' and status='1' ");
    }
    ...
    if($sql->rowCount()>0){
        ...
        foreach ($sql->fetchAll() as $key => $row) {
            // $consultant_id = $row['travel_agent_id'];
